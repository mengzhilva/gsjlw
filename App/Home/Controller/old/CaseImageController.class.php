...
// | * 页面说明：设计师
// +----------------------------------------------------------------------
class CaseImageController extends CommonController {
    ...
    public function  index(){
        ...
        // $housestyle=M('housestyle')->select();
        ...
        $id = I('get.id',0);
        $order = 'id desc';
        $where = 'CID in(select ID from casedecorate where cityID=' .$this->city['ID'].' and status=1) and HID='.$id;
        //$where['status'] = 1;
        ...
        //             break;
        //         case '60-90':
        //             $where['AREA'] =  array('BETWEEN','60 and 90');
        //             $houseareatitle=$_REQUEST['housearea'].'㎡';
        //             break;
        //         case '90-120':
        //             $where['AREA'] =  array('BETWEEN','90 and 120');
        //             $houseareatitle=$_REQUEST['housearea'].'㎡';
        //             break;
        //         case '120-160':
        //             $where['AREA'] =  array('BETWEEN','120 and 160');
        //             $houseareatitle=$_REQUEST['housearea'].'㎡';
        //             break;
        //         case '160-200':
        //             $where['AREA'] =  array('BETWEEN','160 and 200');
        //             $houseareatitle=$_REQUEST['housearea'].'㎡';
        //             break; 
        //         case '200':
        ...
        $casenum = M('case_image')->where($where)->select();
        $total = count($casenum);
        //创建分页对象
        $p = new \Think\Page($total, $numPerPage);

        
        $lists = M('')->table('case_image ci')->field('ci.*,(select BACKNAME from housefunction where ID=ci.HID) as HNAME')->where($where)->order($order)->limit($p->firstRow.','.$p->listRows)->select();
